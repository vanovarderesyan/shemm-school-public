...
const { accessTokenDuration, accessTokenSecretKey, refreshTokenDuration, refreshTokenSecretKey,accessTokenDurationReset,accessTokenDurationResetLong } = config.auth;
 ...
 * @returns {string}
 */
export function generateAccessTokenForReset(data: LoggedInUser): string {
  logger.log('info', 'JWT: Generating access token -', { data, expiresIn: accessTokenDurationReset });

  return jwbt.sign({ data }, refreshTokenSecretKey, { expiresIn: accessTokenDurationReset });
}
 ...
 * @returns {string}
 */
export function generateAccessTokenForResetLong(data: LoggedInUser): string {
  logger.log('info', 'JWT: Generating access token -', { data, expiresIn: accessTokenDurationResetLong });

  return jwbt.sign({ data }, refreshTokenSecretKey, { expiresIn: accessTokenDurationResetLong });
}
